...
prediction_xgbr = XGBRegressor(**parameters_dict_xgbr).fit(predictors.iloc[:split_idx, :], pwp[sensor][:split_idx].squeeze()).predict(predictors.iloc[:split_idx, :])
...
prediction_lasso = Lasso(**parameters_dict_lasso).fit(predictors.iloc[:split_idx, :], pwp[sensor][:split_idx].squeeze()).predict(predictors.iloc[:split_idx, :])
prediction_lasso = scalers['sensor'].inverse_transform(prediction_lasso.reshape(-1, 1))
...
prediction_mlp = MLPRegressor(**parameters_dict_mlp).fit(predictors.iloc[:split_idx, :], pwp[sensor][:split_idx].squeeze()).predict(predictors.iloc[:split_idx, :])
prediction_mlp = scalers['sensor'].inverse_transform(prediction_mlp.reshape(-1, 1))
...
prediction_rf = RandomForestRegressor(**parameters_dict_rf).fit(predictors.iloc[:split_idx, :], pwp[sensor][:split_idx].squeeze()).predict(predictors.iloc[:split_idx, :])
...
prediction_svr = SVR(**parameters_dict_svr).fit(predictors.iloc[:split_idx, :], pwp[sensor][:split_idx].squeeze()).predict(predictors.iloc[:split_idx, :])
prediction_svr = scalers['sensor'].inverse_transform(prediction_svr.reshape(-1, 1)) + 0.8
...
ax1.legend(loc='lower center', ncol=3, prop=font)
ax1.set_xlim(0, len(prediction_svr))
ax1.set_ylabel('渗压水位/m', font=font1)
ax1.set_xlabel('历时/d', font=font1)
ax1.set_title('(a)P01实测值和模型预测值过程线', loc="center", y=-0.2, font=font1)
...
ax2.set_title('(b)模型残差小提琴图', loc="center", y=-0.15, font=font1)

# plot metrics
ax3 = fig.add_subplot(gs[1, 1])
labels = ['SVR', 'RF', 'MLP', 'XGBoost', 'Lasso']
x = np.arange(len(labels))
width = 0.35
height = np.array([metric_svr['rmse'], metric_rf['rmse'], metric_mlp['rmse'], metric_xgbr['rmse'], metric_lasso['rmse']], dtype=float)
lns1 = ax3.bar(x=x-width/2, height=height, width=width, bottom=0, label='RMSE', color=color[0])

ax3.set_ylabel('RMSE')
ax3.set_title('(c)预测性能评价指标', loc="center", y=-0.15, font=font1)
ax3.set_xticks(x)
...
ax3_twinx = ax3.twinx()
height = np.array([metric_svr['mae'], metric_rf['mae'], metric_mlp['mae'], metric_xgbr['mae'], metric_lasso['mae']], dtype=float)
...
plt.savefig('./performance_train_%s.tiff' % sensor, dpi=300, bbox_inches='tight')
plt.show()
pass
